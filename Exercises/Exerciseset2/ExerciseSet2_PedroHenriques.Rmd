...
1. We need 6 factors to explain at least 50% of the variance in the data  
2. The following futures are the ones affecting comp1 the most:  
    - 5 yr T-Notes US  
    - 10 yr T-Notes  
    - Treasury Bonds  
    - 2 yr T-Notes  
    - Canadian Bond  
    - Eurodollar  
3. The following interpretations apply:  
    - Component 1: risk free bonds  
    - Component 2: currencies  
    - Component 3: European stock market indexes  
    - Component 4: North-American stock market indexes  
    - Component 5: European sort term reference rates  
    - Component 6: oil and refined oil assets  
    - Component 7: metals  
    - Component 8: agricultural commodities  
    - Component 9: precious metals  
    - Component 10: Asian stock market indexes  
4. a) Pre-crisis bull market years  
```{r echo=TRUE, tidy=TRUE}
#define data sets
precrisis_data = futures_data[as.Date(rownames(futures_data))>"2002-11-01"&as.Date(rownames(futures_data))<"2007-10-01",]
crisis_data = futures_data[as.Date(rownames(futures_data))>"2007-10-02"&as.Date(rownames(futures_data))<"2009-03-01",]

#create tables for pre crisis
...
Variance_Explained_Table_copy_precrisis<-Variance_Explained_Table_precrisis

rownames(Variance_Explained_Table_precrisis) <- paste("Component", 1:nrow(Variance_Explained_Table_precrisis), sep=" ")
colnames(Variance_Explained_Table_precrisis) <- c("Eigenvalue", "Pct of explained variance", "Cumulative pct of explained variance")

#create chart for pre crisis
iprint.df(round(Variance_Explained_Table_precrisis, 2))
...
```
  ...
  b) Crisis bear market years  
```{r echo=TRUE, tidy=TRUE}
#create tables for crisis
...
Variance_Explained_Table_copy_crisis<-Variance_Explained_Table_crisis

rownames(Variance_Explained_Table_crisis) <- paste("Component", 1:nrow(Variance_Explained_Table_crisis), sep=" ")
colnames(Variance_Explained_Table_crisis) <- c("Eigenvalue", "Pct of explained variance", "Cumulative pct of explained variance")

#create chart for crisis
iprint.df(round(Variance_Explained_Table_crisis, 2))
...
5. During the pre-crisis years the returns on the futures are less correlated than in crisis years. This is an example of the well-kown fact that diversification effects across asset classes tend to be business cycle dependent, i.e. the diversification effect of different asset classes tends to disappear during periods of financial meltdown.    
...
1. The strategy above allocates according to moving averages. If a given futures has been providing higher returns for the past few days (based on its moving average), the strategy will increase its allocation to these futures, and vice versa. It is, in this sense, a purely technical strategy.  
2. We can change the strategy to use a 10 day lag as follows:  
```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq <- seq(10,250,by=20)
...
colnames(signal_used) <- colnames(futures_data)
# Now create the returns of the strategy for each futures time series
strategy_by_future_2 <- scrub(shift(signal_used,10)*futures_data) # use the signal from 2 days ago
# finally, this is our futures trend following strategy
trading_strategy_2 = apply(strategy_by_future_2,1,sum)
names(trading_strategy_2) <- rownames(futures_data)
```

We can see that the results are significantly worse in terms of cumulative returns with a 10 day lag:  

```{r}
iplot.pnl(trading_strategy_2)
```
