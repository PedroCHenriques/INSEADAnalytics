...
**Your Answers here:**  
The Portfolio (https://cran.r-project.org/web/packages/portfolio/index.html) package contains classes for equity portfolio management  
<hr>
...
**Your Answers here:**  
1. We first specify a variable mytickers in line 8 of dataSet1.R which includes both SPY and APPL. Then the code from line 21 onwards extracts both tickers information with "ticker_to_get = mytickers[ticker_index]".  
2. The cumulative returns of APPL during this period is `r round(100*sum(StockReturns[,2]),1)`%. The average daily returns of APPL during this period is `r round(100*mean(StockReturns[,2]),3)`%. The standard deviation of the daily returns of APPL during this period is `r round(100*sd(StockReturns[,2]),3)`%.  
3. We can easily extract Yahoo's stock statistics by inserting YHOO ticker in mytickers as a new collumn. The results could them be extracted by running the same code as in (2):  
 - Cumulative returns of YHOO during this period is `r round(100*sum(StockReturns[,3]),1)`%  
 - Average daily returns of YHOO during this period is `r round(100*mean(StockReturns[,3]),3)`%  
 - Standard deviation of the daily returns of YHOO during this period is `r round(100*sd(StockReturns[,3]),3)`%.  
<hr>
...
For example, we can now quickly estimate the average returns of S&P, Apple and Yahoo (of course this can be done manually, too, but what if we had 500 stocks - e.g. a matrix with 500 columns?) and plot the returns of that 50-50 on S&P, Apple and Yahoo portfolio:
...
2. Based on the help for the R function *apply* (`help(apply)`), can you create again the portfolio of S&P, Apple and Yahoo and plot the returns in a new figure below?

**Your Answers here:**  
1. We can use "ncol(transposedData)" to get the number of collums (which corresponds to `r ncol(transposedData)`) and "nrow(transposedData)" to get the number of rows (which corresponds to `r nrow(transposedData)`).  
2. We can apply the following code o get this portfolio:   
...
**Your Answers here:**  
1. In order to implement these changes we just need to change the start date to 29 December 2000 in dataSet1.R and rerun the code.  
...
Finally, one can read and write data in .CSV files. For example, we can save the first 20 days of data for S&P, Apple and Yahoo in a file using the command:

```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
write.csv(StockReturns[1:20,c("SPY","AAPL","YHOO")], file = "twentydays.csv", row.names = TRUE, col.names = TRUE) 
```
...
**Your Answers here:**  
1. This fuction retrieves the sum of logical tests that are either 1 for "True" or 0 for "False". Since "mydata" is reading the twentydays file and this file is constructed with data from StockReturns it is only natural that the logical test gives us a series of 1 which add up to 20 observations.
